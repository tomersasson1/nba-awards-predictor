...
def train_mip_model(training_csv: Path, val_season: str = "2024-25") -> None:
    df = pd.read_csv(training_csv)
    award_df = df[df["AWARD_TYPE"] == "MIP"].copy()
    if award_df.empty:
        return
    ...
    train_model(model,
                ...
                TrainConfig())
    save_model(model, MODELS_DIR / "mip" / "mip_model.pt")
    save_scaler(scaler, "MIP")
