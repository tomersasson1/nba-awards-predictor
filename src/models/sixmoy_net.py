...
def train_sixmoy_model(training_csv: Path, val_season: str = "2024-25") -> None:
    df = pd.read_csv(training_csv)
    award_df = df[df["AWARD_TYPE"] == "6MOY"].copy()
    if award_df.empty:
        return
    ...
    train_model(model,
                ...
                TrainConfig())
    save_model(model, MODELS_DIR / "6moy" / "6moy_model.pt")
    save_scaler(scaler, "6MOY")
