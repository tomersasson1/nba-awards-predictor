...
def train_roty_model(training_csv: Path, val_season: str = "2024-25") -> None:
    df = pd.read_csv(training_csv)
    award_df = df[df["AWARD_TYPE"] == "ROTY"].copy()
    if award_df.empty:
        return
    ...
    train_model(model,
                ...
                TrainConfig())
    save_model(model, MODELS_DIR / "roty" / "roty_model.pt")
    save_scaler(scaler, "ROTY")
